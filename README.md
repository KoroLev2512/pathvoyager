...
Убедитесь, что PostgreSQL установлен и запущен. База данных будет создана автоматически при первом запуске сервера.

Настройте переменные окружения. Создайте файл `.env` в корне проекта:

```env
# Подключение к PostgreSQL
# Если используете DATABASE_URL, база данных должна существовать заранее
# DATABASE_URL=postgres://user:password@localhost:5432/pathvoyager

# Или используйте отдельные переменные (база данных создастся автоматически):
PGHOST=localhost
PGPORT=5432
PGUSER=postgres
PGPASSWORD=postgres
...
**Примечание:** Если автоматическое создание базы данных не работает, создайте её вручную:

```bash
psql -U postgres -c "CREATE DATABASE pathvoyager;"
```
